...
site_config = {
    ...
    'owner':  ['google-oauth2|112401903840371673242'],  # WER,  Or this can be some aws handle.
    'owner_alias': ['WER'],
    'admin_aliases': [ "WER", "TB", "DH", "KVH", 'KC', "ANS", 'EC'],
    'agent_wms_enc_active':  True,    #True if the agent is used at a site.  For FAT an independent WEMA may not be useful,
                                      #except to monitor a crashed observer
    ...
    'focuser': {
        'focuser1': {
            ...
            'coef_date':  '20211120',    #This appears to be sensible result 44 points -13 to 3C'reference':  6431,    #  Nominal at 10C Primary temperature
            # #F9 setup
        ...
        "filter_wheel1": {
            ...
            'settings': {
                ...
                'filter_data': [['filter', 'filter_index', 'filter_offset', 'sky_gain', 'screen_gain', 'generic'],
                        ...
                        ['PL',   [7,  0],    0, 72.7, [360 , 170], 'PL'],    #0.
                        ['PR',   [0,  8],    0, 11.0, [.32 ,  20], 'PB'],    #1.
                        ['PG',   [0,  7],    0, 18.6, [30  , 170], 'PG'],    #2.
                        ['PB',   [0,  6],    0, 42.3, [360 , 170], 'PR'],    #3.
                        ['HA',   [0,  3],    0, .400, [360 , 170], 'HA'],    #4.
                        ['O3',   [0,  2],    0, 1.84, [360 , 170], 'O3'],    #5.
                        ['S2',   [0,  4],    0, .221, [0.65,  20], 'S2']],   #6.


                'filter_screen_sort':  [6, 4, 5, 2, 3, 4, 1],   #  don't use narrow yet,  8, 10, 9], useless to try.
    ...
    'camera': {
        'camera_1_1': {
            ...
            'settings': {
                ...
                'y_width':  3211,
                'x_chip':  4628,   #  NB Should specify the active pixel area.   20200315 WER
                'y_chip':  3728,
                ...
                'x_pixel':  6,
                'y_pixel':  6,
                'pix_scale': 1.0668_22*4499,     #  asec/pixel F9   0.5751  , F4.9  1.0481         
                ...
                'default_bin':  [1, 1, 0.53],    #  Matched to seeing situation by owner
