...
##****** NB NB NB    These are important, for now, global state variables
...
def illumination(sunRa, sunDec, sunElev, sunDia, moonRa, moonDec, moonElev, moonDia):
    ...
    if sunElev >= -18:
        ...
        x = sunElev / 90.
        i =  10**(il[0] + x * (il[1] + x * (il[2] + x * il[3])) + 2 * log10(sunDia/0.50664))
    else:
        i = 0
    if i < 0: i =0
    f=0
    if moonElev >= -0.8:
        ...
        e = degrees(acos(sin(sunDec) * sin(moonDec) + cos(sunDec) * cos(moonDec) * cos(raDist)))
        f = 180 - e
        if f > 180:
            f = 180
        if f < 0:
            f = 0
        j =  10**(il[0] + x * (il[1] + x * (il[2] + x * il[3])) + (-0.00868 * f - 0.0000000022 * (f ** 4)))
        #Correction for Lunar diameter missing here.
        # + 2*log10(phi/0.951)
    else:
        j = 0
    if j < 0: j = 0
...
def calcEveFlatValues(pWhen, loud=False):
    ptr.date = pWhen
    sun.compute(ptr)
    ...
    print('Eve Flat:  ', FlatStartRa, FlatStartDec, FlatEndRa, FlatEndDec)
