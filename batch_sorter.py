...
def annotate_image(camera_name, archive_path, selector_string, lng_path, out_path):
    ...
    for img in file_list:
        hdu = fits.open(img)
        target = int(hdu[0].header['dec'][:3])
        hdu.close()
        try:
            os.mkdir(archive_path + 'm22/')
            os.mkdir(archive_path + 'p34/')
            os.mkdir(archive_path + 'p53/')
        except:
            pass
        if target in [-22, -23, -24]:
            shutil.move(img,archive_path + 'm22/')

            pass
            #print(-22, target, img)
        elif target in [34, 35, 36]:
            shutil.move(img,archive_path + 'p34')
            pass
            #print(-34, target, img)
        elif target in [53, 54, 55]:
            shutil.move(img,archive_path + 'p53/')
            pass
            # print(-34, target, img)
        else:
            print(img, target)
        continue
    #Get thbbe master images:
    sbHdu = fits.open(lng_path + 'b_2.fits')
    super_bias = sbHdu[0].data.astype('float32')
    pedastal = sbHdu[0].header['PEDASTAL']
    super_bias += pedastal
