...
=======
    #'site_id': 'mrc',  #WER I assume one of these two keys is obsolete.
    'site_location': "Near Santa Barbara, CA. USA",
    'site_description': "Mountain Ranch Camp Observatory",
    'site_dedication':  'Behold the glory of Python and ASCOM!',
    'site_airport_code': "SBA", 
    'TZ_database_name': 'America/Los_Angeles',
    'site_mpc_code':  'NONE',    #This is made up for now.
    'site_time_offset':  -7,     # NB these two should be derived from Python libs so change is automatic
    'site_timezone': 'PDT',      # so I will start sending these in the Weather status


    'site_latitude': 34.459376,     #Decimal degrees, North is Positive
    'site_longitude': -119.681163,   #Decimal degrees, West is negative
    'site_elevation': 317.75,    # meters above sea level
    'site_reference_ambient':  10.0,  #Celsius.  Alternately 12 entries, one for every - mid month.
    'site_reference_pressure':  977.83,  #mbar
    'site_wema_is_a_process':  False,  #True if a real single computer site.
    'site_wema_hostname': 'MRC-WEMA',
    'site_IPC_mechanism': 'redis',   # ['None', shares', 'shelves', 'redis']  Pick One
    'site_redis_ip': '10.15.0.109',  #'127.0.0.1', None if no redis path present,
    'site_debug_flag': False,   #A debugging softie may set this True
    'site_debug_duration_sec': 1800,
    'site_has_specific_code': False,
    
    'observatory_id': ['mrc1', 'mrc2', 'mrc3', 'mcr4', 'mrc5'],
    'enclosure_id':   ['enc1', 'enc1', 'enc2', 'enc3', 'enc4'],
    'enclosure_type': ['clam', 'clam', 'clam', 'dome', 'covr'],
