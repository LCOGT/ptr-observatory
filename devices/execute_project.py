...
class SchedulerObservation:
    ...
    def _take_exposure(self, camera_device, filterwheel_device, time: float, filter_name: str, target_name: str,
                      upload_metadata: dict, smartstack: bool = True, substack: bool = True) -> dict:
        """
    ...
    def get_upload_metadata(self, config: dict) -> dict:
        image_metadata = {
            'size': 'large',
            'INSTRUME': config['instrument_name'],
            'SITEID': self.observation['site'],
            'TELID': self.observation['telescope'],
            'PROPID': self.observation['proposal'],
            'BLKUID': int(self.observation['id']),
            'REQNUM': int(self.observation['request']['id']),
            'OBSTYPE': config['type'],
        }
        return {
    ...
    def _do_instrument_config(self, ic, config, devices, exposure_sequence_done: Callable[[], bool]) -> tuple:
        ...
        try:
            ...
            for _ in range(exposure_count):
                ...
                # Take the exposure
                upload_metadata = self.get_upload_metadata(config)
                expose_result = self._take_exposure(camera, filter_wheel, exposure_time, filter_name, target_name, upload_metadata, smartstack=smartstack, substack=substack)
