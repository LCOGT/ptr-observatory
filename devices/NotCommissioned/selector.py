    ...
    def parse_command(self, command):
        ...
        if action == "new_selection":
            ...
            if 0 <= port <= 3:
                ...
                print(
                    "Active camera was changed to: ",
                    self.config["selector1"]["cameras"][port],
                )
                print(
                    "Active guider was changed to: ",
                    self.config["selector1"]["guiders"][port],
                )
            else:
                print(
                    "Incorrect port specified for instrument selection, at port: ",
                    int(self.selector.GetSwitchValue(0)),
                )
