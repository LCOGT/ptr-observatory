...
class Sequencer:
    ...
    def parse_command(self, command):
        ...
        if action == "run" and script == 'focusAuto':
            ...
            g_dev["obs"].send_to_user(
                "A Stop Script has been called. Cancelling out of running scripts over 30 seconds.")
                ...
                try:
                    ...
                    if ocn_status == None:
                        if self.config['obsid_roof_control'] and not enc_status['shutter_status'] in ['Open', 'open', 'Opening', 'opening'] and g_dev['enc'].mode == 'Automatic'\
                            ...
                            self.enclosure_next_open_time = time.time(
                            ) + (self.config['roof_open_safety_base_time']*60) * g_dev['seq'].opens_this_evening
                    ...
                    if enc_status['shutter_status'] in ['Open', 'open', 'Opening', 'opening']:
                        ...
                        plog(
                            "Failed to open roof/WEMA hasn't opened roof', parking telescope again and sending the close command to the roof.")
    ...
    def park_and_close(self, enc_status):
        ...
        try:
            # and enc_status['shutter_status'] in ['open', ] $ Don't check, just close!
    ...
    def manager(self):
        ...
        if self.weather_report_close_during_evening == True:
            # Don't want scope to cancel all activity during bias/darks etc.
            if ephem_now > self.weather_report_close_during_evening_time and ephem_now < events['Morn Bias Dark']:
                if self.config['obsid_roof_control'] and g_dev['enc'].mode == 'Automatic':
                    self.weather_report_is_acceptable_to_observe = False
        ...
        if not self.bias_dark_latch and ((events['Eve Bias Dark'] <= ephem_now < events['End Eve Bias Dark']) and
            ...
            # Pointing
            req = {'time': self.config['focus_exposure_time'],  'alias':  str(
                self.config['camera']['camera_1_1']['name']), 'image_type': 'focus'}  # NB Should pick up filter and constats from config
...
# =============================================================================
                ...
                if enc_status['enclosure_mode'] in ['Autonomous!', 'Automatic']:
                    ...
                    for block in blocks:  # This merges project spec into the blocks.
                        for project in projects:
                            ...
                            try:
                                ...
                                # nb nb nb 20220920   this faults with 'string indices must be integers". WER
                                block['project'] = None
                    ...
                    for block in blocks:
                        ...
                        if not self.block_guard \
                            ...
                            g_dev['obs'].update()
                            # In this we need to ultimately watch for weather holds.
        ...
        # and g_dev['enc'].mode == 'Automatic' ):
        elif (events['Nightly Reset'] <= ephem_now < events['End Nightly Reset']):
...
#    self.redis_server.set('sim_hold', True, ex=120)
    ...
    def execute_block(self, block_specification):
        ...
        for target in block['project']['project_targets']:  # NB NB NB Do multi-target projects make sense???
            try:
                ...
                dest_dec = float(target['dec']) - float(block_specification['project']
                                                        ['project_constraints']['dec_offset'])
                ...
                plog(e)
                g_dev['obs'].send_to_user(
                    "Could not execute project due to poorly formatted or corrupt project", p_level='INFO')
                continue
            ...
            for exposure in block['project']['exposures']:
                ...
                if multiplex > 1:
                    ...
                    # Do not multiply the count string value as a dict entry!
                    exposure['count'] = int(exposure['count'])*multiplex
            ...
            while left_to_do > 0 and not ended:
                ...
                for exposure in block['project']['exposures']:
                    ...
                    if exposure['area'] in ['300', '300%', 300, '220', '220%', 220, '150', '150%', 150, ]:  # 4 or 5 expsoures.
                        if block_specification['project']['project_constraints']['add_center_to_mosaic']:
                            # Aimpoint + Four mosaic quadrants 36 x 24mm chip
                            offset = [(0.0, 0.0), (-1.5, 1.), (1.5, 1.), (1.5, -1.), (-1.5, -1.)]
                            pane = 0
                        else:
                    ...
                    for displacement in offset:
                        ...
                        if pitch == -1:
                            ...
                            # 0.764243 deg = 0.0509496 Hours  These and pixscale should be computed in config.
                            d_ra = displacement[0]*(pitch)*(x_field_deg/15.)
                        ...
                        if imtype in ['light'] and count > 0:
                            ...
                            if exposure['longstack'] == False:
                                ...
                                longstackswitch = 'yes'
                                longstackname = block_specification['project']['created_at'].replace(
                                    '-', '').replace(':', '')
                            else:
                                longstackswitch = 'no'
                                longstackname = 'no'
                            ...
                            req = {'time': exp_time,  'alias':  str(self.config['camera']['camera_1_1']['name']), 'image_type': imtype, 'smartstack': smartstackswitch,
                                   'longstackswitch': longstackswitch, 'longstackname': longstackname, 'block_end': block['end']}  # NB Should pick up filter and constants from config
                            ...
                            result = g_dev['cam'].expose_command(
                                req, opt, no_AWS=False, solve_it=False, calendar_event_id=calendar_event_id)
                            g_dev['obs'].update()
                            try:
        ...
        while ephem.now() < ending:  # Do not overrun the window end
            ...
            while b_d_to_do > 0:
                ...
                if not single_dark:
                    ...
                    if self.stop_script_called:
                        g_dev["obs"].send_to_user(
                            "Cancelling out of calibration script as stop script has been called.")
                        return
                    b_d_to_do -= 1
                    g_dev['obs'].update()
                    if ephem.now() + (dark_exp_time + cycle_time + 30)/86400 > ending:
    ...
    def collect_and_queue_neglected_fits(self):
        ...
        for fzneglect in bigtokens:
            ...
            # Enqueue into the stream but at the lowest priority ever.
            g_dev['cam'].enqueue_for_AWS(56000000, g_dev['obs'].obsid_path + 'tokens/',
                                         fzneglect.split('tokens')[-1].replace('\\', ''))
            # g_dev['obs'].send_to_aws()
    ...
    def nightly_reset_script(self):
        ...
        if self.config['archive_age'] > 0:
            ...
            for camera in cameras:  # Go through each camera directory
                ...
                plog("in weeks\n")
                g_dev['obs'].send_to_user("Culling " + str(len(deleteDirectories)) +
                                          " from the local archive.", p_level='INFO')
                for entry in range(len(deleteDirectories)):
                    ...
                    try:
                        ...
                        plog("Could not remove: " +
                             str(deleteDirectories[entry]) + ". Usually a file is open in that directory.")
        ...
        if self.config['camera']['camera_1_1']['driver'] == "CCDSoft2XAdaptor.ccdsoft5Camera":
            g_dev['cam'].camera.AutoSavePath = g_dev['obs'].obsid_path + 'archive/' + \
                datetime.datetime.strftime(datetime.datetime.now(), '%Y%m%d')
            try:
                os.mkdir(g_dev['obs'].obsid_path + 'archive/' +
                         datetime.datetime.strftime(datetime.datetime.now(), '%Y%m%d'))
            except:
    ...
    def regenerate_local_masters(self):
        ...
        if len(inputList) == 0 or len(darkinputList) == 0:
            ...
            PLDrive = np.memmap(g_dev['obs'].local_bias_folder + 'tempfile', dtype='float32',
                                mode='w+', shape=(shapeImage[0], shapeImage[1], len(inputList)))
            ...
            fits.writeto(g_dev['obs'].calib_masters_folder + tempfrontcalib +
                         'BIAS_master_bin1.fits', masterBias,  overwrite=True)
            ...
            PLDrive = np.memmap(g_dev['obs'].local_dark_folder + 'tempfile', dtype='float32',
                                mode='w+', shape=(shapeImage[0], shapeImage[1], len(inputList)))
            ...
            fits.writeto(g_dev['obs'].calib_masters_folder + tempfrontcalib +
                         'DARK_master_bin1.fits', masterDark,  overwrite=True)
            ...
            if len(tempfilters) == 0:
                ...
                for filterfolder in tempfilters:
                    ...
                    if len(inputList) == 0:
                        ...
                        PLDrive = np.memmap(g_dev['obs'].local_flat_folder + 'tempfile', dtype='float32',
                                            mode='w+', shape=(shapeImage[0], shapeImage[1], len(inputList)))
                        ...
                        plog("**********************************")
                        plog("Median Stacking each " + str(filtercode) +
                             " flat frame row individually from the Reprojections")
                        plog(datetime.datetime.now().strftime("%H:%M:%S"))
                        # Go through each pixel and calculate nanmedian. Can't do all arrays at once as it is hugely memory intensive
                        finalImage = np.zeros(shapeImage, dtype=float)
                        for xi in range(shapeImage[0]):
                        ...
                        np.save(g_dev['obs'].calib_masters_folder + 'masterFlat_' +
                                str(filtercode) + '_bin1.npy', temporaryFlat)

                        fits.writeto(g_dev['obs'].calib_masters_folder + tempfrontcalib +
                                     'masterFlat_' + str(filtercode) + '_bin1.fits', temporaryFlat, overwrite=True)
    ...
    def sky_flat_script(self, req, opt, morn=False):
        ...
        g_dev['obs'].send_to_user(
            'Sky Flat sequence Starting, Enclosure PRESUMED Open. Telescope should be on sky flat spot.', p_level='INFO')
        evening = not morn
        ...
        collecting_area = self.config['telescope']['telescope1']['collecting_area'] / \
            31808.   # SAF at F4.9 is the reference
        ...
        while len(pop_list) > 0 and ephem.now() < ending and g_dev['obs'].open_and_enabled_to_observe:
            ...
            if g_dev["fil"].null_filterwheel == False:
                ...
                try:
                    _, filt_pointer = g_dev['fil'].set_name_command(
                        {"filter": current_filter}, {})  # 20220825  NB NB NB Chan
                except Exception as e:
            ...
            # and g_dev['enc'].status['shutter_status'] in ['Open', 'open']: # NB NB NB and roof is OPEN! and (ephem_now +3/1440) < g_dev['events']['End Eve Sky Flats' ]:
            while (acquired_count < flat_count):
                ...
                if self.next_flat_observe < time.time():
                    ...
                    try:
                        try:
                            sky_lux = eval(self.redis_server.get('ocn_status'))[
                                'calc_HSI_lux']  # Why Eval, whould have float?
                        except:
                    ...
                    if self.estimated_first_flat_exposure == False:
                        ...
                        if sky_lux != None:
                            if g_dev["fil"].null_filterwheel == False:
                                # g_dev['ocn'].calc_HSI_lux)  #meas_sky_lux)
                                exp_time = target_flat/(collecting_area*sky_lux *
                                                        float(g_dev['fil'].filter_data[filt_pointer][3]))
                                plog('Exposure time:  ', exp_time, scale, sky_lux,
                                     float(g_dev['fil'].filter_data[filt_pointer][3]))
                            else:
                                #exp_time = scale*min_exposure
                                # g_dev['ocn'].calc_HSI_lux)  #meas_sky_lux)
                                exp_time = target_flat/(collecting_area*sky_lux *
                                                        self.config['filter_wheel']['filter_wheel1']['flat_sky_gain'])
                    ...
                    if not g_dev['obs'].open_and_enabled_to_observe:
                        g_dev["obs"].send_to_user(
                            "Cancelling out of activity as no longer open and enabled to observe.")
                        return
                    ...
                    if evening and exp_time > max_exposure:
                        ...
                        plog('Break because proposed evening exposure > maximum flat exposure: ' +
                             str(max_exposure) + ' seconds:  ', exp_time)
                      ...
                      # **************THIS SHOUD BE A WHILE LOOP! WAITING FOR THE SKY TO GET DARK AND EXP TIME TO BE LONGER********************
                        ...
                        if g_dev["fil"].null_filterwheel == False:
                            # nb nb nb BIN CHNAGED FROM 2,2 ON 20220618 wer
                            opt = {'count': 1, 'bin':  1, 'area': 150,
                                   'filter': g_dev['fil'].filter_data[filt_pointer][0]}
                        ...
                        try:
                            self.time_of_next_slew = time.time()
                            fred = g_dev['cam'].expose_command(
                                req, opt, no_AWS=True, do_sep=False, skip_daytime_check=True)
                            if self.stop_script_called:
                                g_dev["obs"].send_to_user(
                                    "Cancelling out of calibration script as stop script has been called.")
                                return
                            ...
                            plog(traceback.format_exc())
                            # NB NB  NB this is drastic action needed.
                            plog("*****NO result returned*****  Will need to restart Camera")
        ...
        # You actually always want it to park, TheSkyX can't stop the telescope tracking, so park is safer... it is before focus anyway.
        g_dev['mnt'].park_command({}, {})
        self.sky_guard = False
    ...
    def screen_flat_script(self, req, opt):
        ...
        for filt in g_dev['fil'].filter_screen_sort:
            ...
            opt = {'area': 100, 'count': 1,
                   'filter': g_dev['fil'].filter_data[filter_number][0], 'hint': 'screen pre-filter dark'}
            result = g_dev['cam'].expose_command(req, opt, no_AWS=True, skip_open_check=True, skip_daytime_check=True)
            if self.stop_script_called:
                g_dev["obs"].send_to_user("Cancelling out of calibration script as stop script has been called.")
                return
            ...
            g_dev['obs'].update()
            req = {'time': float(exp_time),  'alias': camera_name, 'image_type': 'screen flat'}
            opt = {'area': 100, 'count': flat_count,
                   'filter': g_dev['fil'].filter_data[filter_number][0], 'hint': 'screen filter light'}
            result = g_dev['cam'].expose_command(req, opt, no_AWS=True, skip_open_check=True, skip_daytime_check=True)
            if self.stop_script_called:
                g_dev["obs"].send_to_user("Cancelling out of calibration script as stop script has been called.")
                return
            ...
            req = {'time': float(exp_time),  'alias': camera_name, 'image_type': 'screen flat'}
            opt = {'area': 100, 'count': 1,
                   'filter': g_dev['fil'].filter_data[filter_number][0], 'hint': 'screen post-filter dark'}
            result = g_dev['cam'].expose_command(req, opt, no_AWS=True, skip_open_check=True, skip_daytime_check=True)
            if self.stop_script_called:
                g_dev["obs"].send_to_user("Cancelling out of calibration script as stop script has been called.")
                return
...
# =============================================================================
        ...
        # Read these to go back.  NB NB Need to cleanly pass these on so we can return to proper target.
...
# =============================================================================
        ...
        if True:  # req2['target'] == 'near_tycho_star':   ## 'bin', 'area'  Other parameters
            ...
            self.focus_catalogue_skycoord = SkyCoord(
                ra=self.focus_catalogue[:, 0]*u.deg, dec=self.focus_catalogue[:, 1]*u.deg)
            ...
            for ctr in range(len(self.focus_catalogue_altitudes)):
                if self.focus_catalogue_altitudes[ctr].alt / u.deg > 45.0:
                    above_altitude_patches.append(
                        [self.focus_catalogue[ctr, 0], self.focus_catalogue[ctr, 1], self.focus_catalogue[ctr, 2]])
            above_altitude_patches = np.asarray(above_altitude_patches)
            self.focus_catalogue_skycoord = SkyCoord(
                ra=above_altitude_patches[:, 0]*u.deg, dec=above_altitude_patches[:, 1]*u.deg)
            ...
            try:
                plog("Going to near focus patch of " + str(focus_patch_n) +
                     " 9th to 12th mag stars " + str(d2d.deg) + "  degrees away.")
        ...
        try:
            ...
            while g_dev['foc'].focuser.IsMoving or \
                ...
                if g_dev['foc'].focuser.IsMoving:
                    st += 'Waiting for Focuser to shift.\n'
                if g_dev['mnt'].mount.Slewing:
                    st += 'Waiting for Mount to Slew\n'
                #if g_dev['enc'].status['dome_slewing']: st += 'd>'
                if rot_report == 0:
                    plog(st)
                    st = ""
        ...
        # NB added 20220209 Nasty bug, varies with prior state
        g_dev['foc'].guarded_move((foc_pos0 - 0 * throw)*g_dev['foc'].micron_to_steps)
        ...
        while retry < 3:
            if not sim:
                g_dev['obs'].scan_requests()
                # , script = 'auto_focus_script_0')  #  This is where we start.
        ...
        if not sim:
            g_dev['obs'].scan_requests()
            # , script = 'auto_focus_script_1')  #  This is moving in one throw.
            result = g_dev['cam'].expose_command(req, opt, no_AWS=True, solve_it=False)
            if self.stop_script_called:
                g_dev["obs"].send_to_user("Cancelling out of autofocus script as stop script has been called.")
                return
       ...
       # time.sleep(10)#It is important to overshoot to overcome any backlash  WE need to be sure Exposure waits.
        ...
        if not sim:
            g_dev['obs'].scan_requests()
            # , script = 'auto_focus_script_2')  #  This is moving out one throw.
            result = g_dev['cam'].expose_command(req, opt, no_AWS=True, solve_it=False)
            if self.stop_script_called:
                g_dev["obs"].send_to_user("Cancelling out of autofocus script as stop script has been called.")
                return
        ...
        if spot1 is None or spot2 is None or spot3 is None or spot1 == False or spot2 == False or spot3 == False:  # New additon to stop crash when no spots
            ...
            # NB NB 20221002 THis unit fix shoudl be in the routine. WER
            g_dev['foc'].guarded_move((focus_start)*g_dev['foc'].micron_to_steps)
            self.sequencer_hold = False  # Allow comand checks.
            self.af_guard = False
            ...
            try:
                ...
                self.sequencer_hold = False  # Allow comand checks.
                self.af_guard = False
                # NB NB Does this really take us back to starting point?
            ...
            if min(x) <= d1 <= max(x):
                ...
                if not sim:
                    g_dev['obs'].scan_requests()
                    # script = 'auto_focus_script_3')  #  This is verifying the new focus.
                    result = g_dev['cam'].expose_command(req, opt, no_AWS=True, solve_it=False)
                    if self.stop_script_called:
                        g_dev["obs"].send_to_user("Cancelling out of autofocus script as stop script has been called.")
                        return
                ...
                g_dev['obs'].send_to_user('Found best focus at:  ' + str(foc_pos4) +
                                          ' measured FWHM is:  ' + str(round(spot4, 2)), p_level='INFO')
            ...
            plog('X, Y:  ', x, y, 'Desire center to be smallest.')
            g_dev['obs'].send_to_user('X, Y:  ' + str(x) + " " + str(y) +
                                      ' Desire center to be smallest.', p_level='INFO')
    ...
    def extensive_focus_script(self, req, opt, throw=700, begin_at=None, no_auto_after_solve=False):
        ...
        extensive_focus = []
        for ctr in range(4):
            g_dev['foc'].guarded_move((foc_pos0 - (ctr+0)*throw) *
                                      g_dev['foc'].micron_to_steps)  # Added 20220209! A bit late
            ...
            try:
                ...
                if np.isnan(result['FWHM']):
                    req = {'time': 2*float(self.config['focus_exposure_time']),  'alias':  str(
                        self.config['camera']['camera_1_1']['name']), 'image_type': 'focus'}  # NB Should pick up filter and constats from config
                    opt = {'area': 100, 'count': 1, 'filter': 'focus'}
                    result = g_dev['cam'].expose_command(req, opt, no_AWS=True, solve_it=False)
                    if self.stop_script_called:
        ...
        for ctr in range(3):
            g_dev['foc'].guarded_move((foc_pos0 + (ctr+1)*throw) *
                                      g_dev['foc'].micron_to_steps)  # Added 20220209! A bit late
            # throw = 100  # NB again, from config.  Units are microns
            if not sim:
                g_dev['obs'].scan_requests()
            ...
            try:
                ...
                if np.isnan(result['FWHM']):
                    req = {'time': 3*float(self.config['focus_exposure_time']),  'alias':  str(
                        self.config['camera']['camera_1_1']['name']), 'image_type': 'focus'}  # NB Should pick up filter and constats from config
                    opt = {'area': 100, 'count': 1, 'filter': 'focus'}
                    result = g_dev['cam'].expose_command(req, opt, no_AWS=True, solve_it=False)
                    if self.stop_script_called:
    ...
    def coarse_focus_script(self, req, opt, throw=700, begin_at=None):
        ...
        if True:  # req['target'] == 'near_tycho_star':   ## 'bin', 'area'  Other parameters
            ...
            g_dev['mnt'].go_coord(focus_patch_ra, focus_patch_dec)
            req = {'time': self.config['focus_exposure_time'],  'alias':  str(
                self.config['camera']['camera_1_1']['name']), 'image_type': 'auto_focus'}  # NB Should pick up filter and constats from config
            opt = {'area': 100, 'count': 1, 'filter': 'focus'}
        else:
            pass  # Just take time image where currently pointed.
            req = {'time': self.config['focus_exposure_time'],  'alias':  str(
        ...
        if min(x) <= d1 <= max(x):
            ...
            try:
                ...
                g_dev['obs'].send_to_user("Found best focus at: " + str(foc_pos4) +
                                          ' measured FWHM is: ' + str(round(spot6, 2)), p_level='INFO')
            except:
            ...
            g_dev['obs'].send_to_user(
                'Coarse_focus did not converge. Moving back to starting focus:  ' + str(foc_pos0), p_level='INFO')
    ...
    def run_nightly_weather_report(self):
        ...
        for hourly_report in one_call.forecast_hourly:
            ...
            if hourcounter > hours_until_end_of_observing:
                ...
                fitzgerald_weather_number_grid.append([hourly_report.humidity, hourly_report.clouds, hourly_report.wind()[
                                                      'speed'], hourly_report.status, hourly_report.detailed_status])
    ...
    def centering_exposure(self):
        ...
        req = {'time': self.config['pointing_exposure_time'],  'alias':  str(
            self.config['camera']['camera_1_1']['name']), 'image_type': 'light'}  # NB Should pick up filter and constats from config
        opt = {'area': 100, 'count': 1, 'filter': 'focus'}
