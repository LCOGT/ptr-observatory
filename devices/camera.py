...
################################################# HERE IS WHERE IN-LINE STUFF HAPPENS.
                ...
                if focus_image == True:
                    ...
                    try:
                        ...
                        #         # 1) Mask out any nan or infinite radii
                        #         valid = np.isfinite(r)
                        #         r_valid     = r[valid]
                        #         fluxes_valid = fluxes[valid]
                                
                        #         # 2) Floor & cast to int safely
                        #         bins = np.floor(r_valid).astype(int)
                                
                        #         # 3) (Optional) Clip any remaining negatives to zero
                        #         bins = np.clip(bins, 0, None)
                                
                        #         # 4) Now build your profile
                        #         num = np.bincount(bins, weights=fluxes_valid)
                        #         den = np.bincount(bins)
                        #         rad_prof = num / den
                                
                        #         #print("bin range:", bins.min(), bins.max())
                            
                        #         # # after computing bins and fluxes:
                        ...
                        #     ax.imshow(outputimg, cmap='gray', origin='lower')
                        #     ax.set_title("Detected donuts with measured diameters")
                        #     ax.set_xlabel("X (pixels)")
                        #     ax.set_ylabel("Y (pixels)")
                            
                        #     for p, diameter in zip(largest, diameter_peaks):
                        #         # regionprops gives centroid as (row, col) = (y, x)
                        #         y0, x0 = p.weighted_centroid
                        #         radius = diameter / 2.0
                            
                        #         # draw the ring
                        #         circ = Circle((x0, y0), radius,
                        #                       edgecolor='red', facecolor='none',
                        #                       linewidth=1.5)
                        #         ax.add_patch(circ)
                            
                        #         # annotate the diameter
                        #         ax.text(x0 + radius + 2, y0,
                        #                 f"{diameter:.1f}px",
                        ...
                        # # 1) Detect edges
                        # edges = canny(outputimg, sigma=3, low_threshold=0.1, high_threshold=0.3)
                        
                        # # 2) Hough‐transform: look for circles over a plausible radius range
                        # radii = np.arange(20, 200, 5)   # adjust min/max to bracket your donuts
                        # hough_res = hough_circle(edges, radii)
                        
                        # # 3) Pick the top N circles
                        # accums, cx, cy, rad = hough_circle_peaks(
                        #     hough_res, radii, total_num_peaks=50, min_xdistance=20, min_ydistance=20
                        # )
                        
                        # # 4) Plot
                        # fig, ax = plt.subplots(figsize=(10,10))
                        # ax.imshow(outputimg, cmap='gray', origin='lower')
                        # ax.set_title("Hough‐detected donuts")
                        # for x0, y0, r0 in zip(cx, cy, rad):
                        #     c = Circle((x0, y0), r0, edgecolor='red', facecolor='none', linewidth=1.5)
                        #     ax.add_patch(c)
                        #     ax.text(x0 + r0 + 3, y0, f"{2*r0:.0f}px", color='yellow', fontsize=8)
                        ...
                        # 1) Lightly smooth to suppress pixel noise
                        img_blur = gaussian_filter(outputimg, sigma=2)
                        
                        # 2) Compute a gradient‐magnitude edge map
                        grad = sobel(img_blur)
                        
                        # 3) Binarize to pick out just the bright ring edges
                        th = np.percentile(grad, 95)     # top 5% of gradient = edges
                        edges = grad > th                # bool array
                        
                        # 4) Distance‐transform the *inverted* edge map
                        #    now every pixel’s value = distance to nearest edge
                        dt = distance_transform_edt(~edges)
                        
                        # 5) Find local maxima in dt → circle centers
                        #    enforce min_distance~=expected min radius so they don’t crowd
                        min_radius = 10    # px, adjust to your smallest donut
                        ...
                        # 6) Extract radii = dt[y,x]
                        radii = dt[coords[:,0], coords[:,1]]
                        
                        # 7) Optionally keep only the 50 largest
                        idxs = np.argsort(radii)[::-1][:50]
                        centers = coords[idxs]
                        radii   = radii[idxs]
                        ...
                        fig, ax = plt.subplots(figsize=(10,10))
                        ax.imshow(outputimg, cmap='gray', origin='lower')
                        ax.set_title("Detected donuts via distance‐transform")
                        for (y0, x0), r in zip(centers, radii):
                            circ = Circle((x0, y0), r,
                            ...
                            ax.text(x0 + r + 2, y0,
                                    f"{2*r:.0f}px",
                                    color='yellow', fontsize=8)
                        ax.set_xlim(0, outputimg.shape[1])
                        ax.set_ylim(0, outputimg.shape[0])
                        ax.set_xlabel("X (px)")
                        ax.set_ylabel("Y (px)")
                        plt.tight_layout()
                        plt.show()
