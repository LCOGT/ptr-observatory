...
from astropy.coordinates import SkyCoord, EarthLocation, AltAz, get_sun, \
                                get_moon, FK5, FK4, ICRS, Distance
from ptr_utility import plog
import short_utility
import math
import ephem
...
class Mount:
    ...
    def __init__(self, driver: str, name: str, settings: dict, config: dict, astro_events, tel=False):
        ...
        #Pick up early from Config, but note some drivers turn these to False
        try:
        ...
        if "ASCOM.SoftwareBisque.Telescope" in driver:
            self.theskyx = True
            self.refr_on = False    #Presumably handled in SB code.
            self.model_on = False
            self.rates_on = False
        else:
            self.theskyx = False
        ...
        if "PWI4" in driver:
            self.is_pwi4 = True
            self.refr_on = False    #Presumably handled in PWI4 code.
            self.model_on = False
            self.rates_on = False
        else:
            self.is_pwi4 = False
        ...
        #20240323  These are all 0.0 and are obsoleted if model is on.
        if self.model_on:
            self.west_clutch_ra_correction = 0.0
            self.west_clutch_dec_correction = 0.0
            self.east_flip_ra_correction = 0.0
            self.east_flip_dec_correction = 0.0
        else:
    ...
    #        plog("Paddle  not operational??")
        ...
        # initialisation values     ??Why not have these reflect the telescope's actual values?
        self.alt= 45
        self.airmass = 1.5
        self.az = 160
        self.zen = 45
        ...
        self.slewtoRA = 1.0     #??why these values?
        self.slewtoDEC = 34.0
...
# =============================================================================
#                                 Here is where all external goto calls end up trigering a slew -- except home and park.
# =============================================================================
    ...
    def get_current_times(self):
        ...
        self.equinox_now = 'J' +str(round((iso_day[0] + ((iso_day[1]-1)*7 + (iso_day[2] ))/365.), 3))
        return
    ...
    def go_command(self, skyflatspot=None, ra=None, dec=None, az=None, alt=None, ha=None, \
        ...
        try:
            ...
            if skyflatspot != None:
                ...
                dec = float(dec)
                breakpoint()
                #az, alt = ptr_utility.transform_haDec_to_azAlt(ha, dec, lat=self.config['latitude'])  #NB The reference to latitude does not work.
                temppointing = AltAz(location=self.site_coordinates, obstime=Time.now(), alt=alt*u.deg, az=az*u.deg)
                altazskycoord=SkyCoord(alt=alt*u.deg, az=az*u.deg, obstime=Time.now(), location=self.site_coordinates, frame='altaz')
                ra = altazskycoord.icrs.ra.deg /15
                dec = altazskycoord.icrs.dec.deg
        ...
        while successful_move == 0:
            try:
                ...
                # if self.model_on:
                #     breakpoint()
                #   #OK here is  NOT where we convert to the mount coordinate system

                self.slewtoRA = ra
                self.slewtoDEC = dec
# =============================================================================
#                 #NB THIS command starts a GOTO
# =============================================================================
                self.slewtoAsyncRequested=True
                self.wait_for_mount_update()
    ...
    # NB 20240323  These wil ltend to become more obsolete, or have small values once the model is working properly.
